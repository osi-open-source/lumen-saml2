...
class Saml2Auth
{
     ...
     *
     * @param string $idpName The target IDP name, must correspond to config file
     *     'config/saml2/${idpName}_idp_settings.php'
    ...
    public static function loadOneLoginAuthFromIpdConfig($idpName)
    {
        ...
        if (strpos($config['sp']['privateKey'], 'file://') === 0) {
            $config['sp']['privateKey'] = self::extractPkeyFromFile($config['sp']['privateKey']);
        }
        if (strpos($config['sp']['x509cert'], 'file://') === 0) {
            $config['sp']['x509cert'] = self::extractCertFromFile($config['sp']['x509cert']);
        }
        if (strpos($config['idp']['x509cert'], 'file://') === 0) {
            $config['idp']['x509cert'] = self::extractCertFromFile($config['idp']['x509cert']);
        }
